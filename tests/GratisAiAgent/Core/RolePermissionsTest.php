...
class RolePermissionsTest extends WP_UnitTestCase {
	...
	public function test_option_name_constant(): void {
		$this->assertSame( 'gratis_ai_agent_role_permissions', RolePermissions::OPTION_NAME );
	}
	...
	public function test_get_merges_saved_over_defaults(): void {
		update_option(
			...
			[
				'editor' => [
					'chat_access'       => false,
					'allowed_abilities' => [ 'gratis-ai-agent/memory-save' ],
				],
			]
		);
		...
		$this->assertSame( [ 'gratis-ai-agent/memory-save' ], $result['editor']['allowed_abilities'] );
	...
	public function test_role_with_specific_abilities_returns_list(): void {
		update_option(
			...
			[
				'author' => [
					'chat_access'       => true,
					'allowed_abilities' => [ 'gratis-ai-agent/memory-save', 'gratis-ai-agent/post-create' ],
				],
			]
		);
		...
		$this->assertContains( 'gratis-ai-agent/memory-save', $allowed );
		$this->assertContains( 'gratis-ai-agent/post-create', $allowed );
	}
	...
	public function test_administrator_can_use_any_ability(): void {
		...
		$this->assertTrue( RolePermissions::current_user_can_use_ability( 'gratis-ai-agent/any-ability' ) );
	}
	...
	public function test_user_can_use_allowed_ability(): void {
		...
		$this->assertTrue( RolePermissions::current_user_can_use_ability( 'gratis-ai-agent/memory-save' ) );
	}
	...
	public function test_user_cannot_use_disallowed_ability(): void {
		...
		$this->assertFalse( RolePermissions::current_user_can_use_ability( 'gratis-ai-agent/file-write' ) );
	}
