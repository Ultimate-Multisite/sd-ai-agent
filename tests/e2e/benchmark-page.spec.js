 ...
 *   tools.php?page=sdaa-benchmark
 *
 * It renders BenchmarkPageApp inside #sdaa-benchmark-root and
 * provides:
...
test.describe( 'Benchmark Page - Page Render', () => {
	test.beforeEach( async ( { page } ) => {
		...
		// an explicit wait for the PHP-rendered .sdaa-benchmark-wrap so the
		// test always sees the page before asserting on content inside it.
		await expect(
			page.locator( '.sdaa-benchmark-wrap' )
		).toBeVisible( { timeout: 30_000 } );
	} );

	test( 'benchmark page renders the wrap and heading', async ( { page } ) => {
		// ModelBenchmarkPage::render() outputs .sdaa-benchmark-wrap
		// with an h1 "Model Benchmark".
		const wrap = page.locator( '.sdaa-benchmark-wrap' );
	...
	test( 'benchmark React app mounts inside the root container', async ( {
		...
		// BenchmarkPageApp mounts into #sdaa-benchmark-root and
		// renders .sdaa-benchmark-page as its outer wrapper.
		await expect(
			page.locator( '#sdaa-benchmark-root' )
		).toBeVisible();

		await expect(
			page.locator( '.sdaa-benchmark-page' )
		).toBeVisible();
	} );
	...
	test( 'tab panel renders New Benchmark and History tabs', async ( {
		...
		const tabPanel = page.locator( '.sdaa-benchmark-tabs' );
...
test.describe( 'Benchmark Page - Form Inputs', () => {
	...
	test( 'Model Selector renders provider groups with checkboxes', async ( {
		...
		// ModelSelector renders .sdaa-model-selector with provider groups.
		// The built-in "WordPress AI Client" group is always present.
		const modelSelector = page.locator( '.sdaa-model-selector' );
	...
	test( 'selecting a model enables it in the model selector', async ( {
		...
		const counter = page.locator( '.sdaa-model-selector-actions span' );
...
test.describe( 'Benchmark Page - Run List', () => {
	...
	test( 'History tab is clickable and shows the run list', async ( {
		...
		// RunList renders .sdaa-benchmark-run-list when runs exist.
		await expect(
			page.locator( '.sdaa-benchmark-run-list' )
	...
	test( 'run list table shows View and Delete action buttons', async ( {
		...
		const runList = page.locator( '.sdaa-benchmark-run-list' );
	...
	test( 'run list shows status badge for each run', async ( { page } ) => {
		...
		// RunList renders a .sdaa-benchmark-status span for each run.
		const statusBadge = page
			.locator( '.sdaa-benchmark-status' )
			.first();
...
test.describe( 'Benchmark Page - Empty Run List', () => {
	...
	test( 'History tab shows empty state when no runs exist', async ( {
		...
		// RunList renders .sdaa-benchmark-empty with "No benchmark runs yet."
		await expect(
			page.locator( '.sdaa-benchmark-empty' )
		).toBeVisible( { timeout: 10_000 } );
