...
async function screenshot( page, name ) {
	const path = `${ SCREENSHOT_DIR }/${ String( stepNum ).padStart(
		2,
		'0'
	) }-${ name }.png`;
...
async function main() {
	...
	try {
		...
		await page.goto( `${ BASE_URL }/wp-admin/network/plugins.php`, {
			waitUntil: 'networkidle',
			timeout: 15000,
		} );
		...
		const aiAgentActive =
			pageContent.includes( 'ai-agent/ai-agent.php' ) &&
			( pageContent.includes( 'Network Deactivate' ) ||
				pageContent.includes( 'network-deactivate' ) );
		log( `AI Agent plugin network active: ${ aiAgentActive }` );

		// Check OpenAI Compatible Connector
		const connectorActive =
			pageContent.includes( 'openai-compatible' ) &&
			( pageContent.includes( 'Network Deactivate' ) ||
				pageContent.includes( 'network-deactivate' ) );
		log(
			`OpenAI Compatible Connector network active: ${ connectorActive }`
		);
		...
		if ( ! aiAgentActive ) {
			...
			const activateLink = page.locator(
				'tr[data-plugin="ai-agent/ai-agent.php"] .activate a, tr#ai-agent a[href*="action=activate"]'
			);
			if ( ( await activateLink.count() ) > 0 ) {
				...
				for ( let i = 0; i < count; i++ ) {
					...
					if (
						...
						! text.includes( 'Network Deactivate' )
					) {
						const activateBtn = rows
							.nth( i )
							.locator( 'a[href*="action=activate"]' );
		...
		if ( ! connectorActive ) {
			log(
				'Attempting to network activate OpenAI Compatible Connector...'
			);
			const rows = page.locator( 'tr' );
			const count = await rows.count();
			for ( let i = 0; i < count; i++ ) {
				...
				if (
					...
					if ( ( await activateBtn.count() ) > 0 ) {
						...
						await page.waitForLoadState( 'networkidle' );
						log(
							'OpenAI Compatible Connector activated via row search'
						);
						break;
					}
				}
		...
		if ( ! foundSettingsPage ) {
			...
			const relevantMenus = menuLinks.filter(
				( m ) =>
		...
		const configResult = await page.evaluate( async () => {
			...
			try {
				...
				if ( resp.ok ) {
					...
					results.currentSettings = Object.keys( settings ).filter(
						( k ) =>
							k.includes( 'openai' ) ||
			...
			try {
				const resp = await fetch( '/?rest_route=/wp/v2/settings', {
					...
					body: JSON.stringify( {
						openai_compatible_connector_endpoint:
							'https://api.synthetic.new/openai/v1',
						openai_compatible_connector_api_key:
							'syn_7eb697227c00701e38e6be1a1e5ba3d3',
					} ),
				} );
		...
		log(
			`REST API config result: ${ JSON.stringify(
				configResult
			).substring( 0, 500 ) }`
		);
		...
		log(
			`AJAX config result: ${ JSON.stringify( ajaxResult ).substring(
				0,
				300
			) }`
		);
		...
		const optionCheck = await page.evaluate( async () => {
			try {
				...
				const connectorRoutes = Object.keys(
					routes.routes || {}
				).filter(
					( r ) =>
						r.includes( 'connector' ) ||
						r.includes( 'openai' ) ||
						r.includes( 'ai-agent' )
				);
		...
		for ( const url of settingsPages ) {
			...
			if (
				...
				const reactContent = await page
					.locator(
						'#ai-agent-settings-root, #ai-agent-root, [data-wp-component], .components-select-control'
					)
					.evaluateAll( ( els ) =>
						els.map( ( el ) => el.innerHTML.substring( 0, 500 ) )
					);
				log(
					`React content: ${ JSON.stringify( reactContent ).substring(
						0,
						500
					) }`
				);
		...
		if ( ! foundAgentSettings ) {
			log(
				'AI Agent Settings page not found at expected URLs. Checking admin menu...'
			);
			await page.goto( `${ BASE_URL }/wp-admin/`, {
				waitUntil: 'networkidle',
				timeout: 10000,
			} );
			const allMenus = await page
				.locator( '#adminmenu a' )
				.evaluateAll( ( els ) =>
					els
						.map( ( el ) => ( {
			...
			for ( const menu of allMenus ) {
				if (
					menu.href &&
					menu.text.toLowerCase().includes( 'setting' )
				) {
			...
			if (
				...
				const chatUI = await page.evaluate( () => {
					const root = document.querySelector(
						'#ai-agent-root, #ai-agent-app, [class*="chat"], [class*="agent"]'
					);
					return {
						rootFound: !! root,
						rootId: root?.id,
						...
						scripts: Array.from(
							...
							.map( ( s ) => s.src )
							.filter( ( s ) => s.includes( 'ai-agent' ) ),
					};
				} );
				log(
					`Chat UI state: ${ JSON.stringify( chatUI ).substring(
						0,
						800
					) }`
				);
		...
		if ( foundChatPage ) {
						...
						if ( ( await btn.count() ) > 0 ) {
							...
							const responseCheck = await page.evaluate( () => {
								const messages = document.querySelectorAll(
									'[class*="message"], [class*="Message"], [class*="response"], [class*="Response"], [class*="bubble"], [class*="chat-item"]'
								);
								return {
									...
									allText: document
										.querySelector(
											'#ai-agent-root, #ai-agent-app, [class*="chat"]'
										)
										?.textContent?.substring( 0, 1000 ),
								};
							} );
							log(
								`Response check: ${ JSON.stringify(
									responseCheck
								).substring( 0, 500 ) }`
							);

							if (
								! responseCheck.lastMessage ||
								responseCheck.messageCount < 2
							) {
								// Wait longer
								...
								log(
									`Response check (20s): ${ JSON.stringify(
										responseCheck2
									).substring( 0, 500 ) }`
								);
							}
			...
			if ( ! inputFound ) {
				...
				const pageStructure = await page.evaluate( () => {
					return document
						.querySelector( '#wpbody-content' )
						?.innerHTML?.substring( 0, 2000 );
				} );
				log(
					`Page structure: ${ pageStructure?.substring( 0, 800 ) }`
				);
			}
		}
		...
		const apiResult = await page.evaluate( async () => {
			...
			results.nonce =
				window.wpApiSettings?.nonce?.substring( 0, 10 ) + '...';
			...
			try {
				const resp = await fetch(
					'/?rest_route=/ai-agent/v1/sessions',
					{
						method: 'POST',
						headers: {
							'Content-Type': 'application/json',
		...
		log(
			`API test results: ${ JSON.stringify(
				apiResult,
				null,
				2
			).substring( 0, 2000 ) }`
		...
		if ( apiResult.sessionJson?.id || apiResult.sessionJson?.session_id ) {
			const sessionId =
				apiResult.sessionJson.id || apiResult.sessionJson.session_id;
			log(
				`Got session ID: ${ sessionId }. Trying to send message with session...`
			);

			const sessionMsgResult = await page.evaluate( async ( sid ) => {
				try {
					const resp = await fetch(
						`/?rest_route=/ai-agent/v1/sessions/${ sid }/messages`,
						{
							method: 'POST',
							headers: {
								'Content-Type': 'application/json',
			...
			log(
				`Session message result: ${ JSON.stringify(
					sessionMsgResult
				).substring( 0, 1000 ) }`
			);
		}
		...
		try {
			const debugResp = await page.goto(
			...
			if ( debugResp && debugResp.status() === 200 ) {
				...
				log(
					`debug.log content (last 500 chars): ${ debugContent?.slice(
						-500
					) }`
				);
			}
