 ...
 * - Renders sdaa-chart-block wrapper for valid config
 * - Renders sdaa-chart-error wrapper for invalid config
 *
...
describe( 'ChartBlock rendering', () => {
	test( 'renders sdaa-chart-block wrapper for valid config', () => {
		const html = renderToStaticMarkup(
			createElement( ChartBlock, {}, VALID_CONFIG )
		);
		expect( html ).toContain( 'sdaa-chart-block' );
	} );
	...
	test( 'does not render error wrapper for valid config', () => {
		...
		expect( html ).not.toContain( 'sdaa-chart-error' );
	} );

	test( 'renders sdaa-chart-error wrapper for invalid JSON', () => {
		const html = renderToStaticMarkup(
			createElement( ChartBlock, {}, INVALID_JSON )
		);
		expect( html ).toContain( 'sdaa-chart-error' );
	} );
