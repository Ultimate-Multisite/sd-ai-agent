	...
	public function sendRequest( RequestInterface $request ): ResponseInterface {
		...
		if ( is_wp_error( $response ) ) {
			$message = sprintf(
				...
				__( 'Network error occurred while sending %1$s request to %2$s: %3$s', 'superdav-ai-agent' ),
				$request->getMethod(),
				$url,
	...
	public function sendRequestWithOptions( RequestInterface $request, RequestOptions $options ): ResponseInterface {
		...
		if ( is_wp_error( $response ) ) {
			$message = sprintf(
				...
				__( 'Network error occurred while sending request to %1$s: %2$s', 'superdav-ai-agent' ),
				$url,
				$response->get_error_message()
			);
