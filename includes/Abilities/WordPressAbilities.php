...
class InstallPluginAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( empty( $slug ) ) {
			return new WP_Error( 'sd_ai_agent_empty_slug', __( 'Plugin slug is required.', 'gratis-ai-agent' ) );
		}
		...
		if ( false === $result ) {
			...
			return new WP_Error( 'sd_ai_agent_install_failed', __( 'Installation failed for unknown reason.', 'gratis-ai-agent' ) );
		}
...
class UpdatePluginAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( '' === $slug && '' === $plugin_file ) {
			return new WP_Error( 'sd_ai_agent_missing_plugin', __( 'Either "slug" or "plugin_file" is required.', 'gratis-ai-agent' ) );
		}
		...
		if ( false === $result ) {
			...
			return new WP_Error( 'sd_ai_agent_update_failed', __( 'Plugin update failed for unknown reason.', 'gratis-ai-agent' ) );
		}
		...
		if ( ! in_array( $function, $allowed, true ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'The function "%s" is not in the allowed list. Use the sd_ai_agent_allowed_wp_functions filter to extend it.', 'gratis-ai-agent' ),
					$function
				)
			);
		}
...
class InstallPluginFromUrlAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( '' === $url ) {
			return new WP_Error( 'sd_ai_agent_empty_url', __( 'A plugin ZIP URL is required.', 'gratis-ai-agent' ) );
		}
...
class DeletePluginAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( false === $result ) {
			return new WP_Error( 'sd_ai_agent_delete_failed', __( 'Plugin deletion failed for unknown reason.', 'gratis-ai-agent' ) );
		}
...
class SearchPluginDirectoryAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( '' === $search ) {
			return new WP_Error( 'sd_ai_agent_empty_search', __( 'A search keyword is required.', 'gratis-ai-agent' ) );
		}
...
class SwitchPluginAbility extends AbstractAbility {
	...
	protected function execute_callback( $input ) {
		...
		if ( '' === $activate_target ) {
			return new WP_Error( 'sd_ai_agent_missing_activate', __( '"activate" is required.', 'gratis-ai-agent' ) );
		}
