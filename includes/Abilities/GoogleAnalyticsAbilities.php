...
trait GaApiClient {
	...
	private function load_credentials() {
		...
		if ( empty( $creds['property_id'] ) ) {
			return new WP_Error(
				'ga_no_property_id',
				__( 'Google Analytics property ID is not configured. Go to Settings > Superdav AI Agent Settings > Integrations to add your GA4 property ID and service account key.', 'gratis-ai-agent' )
			);
		}
		...
		if ( empty( $creds['service_account_json'] ) ) {
			return new WP_Error(
				'ga_no_credentials',
				__( 'Google Analytics service account JSON is not configured. Go to Settings > Superdav AI Agent Settings > Integrations to add your service account key.', 'gratis-ai-agent' )
			);
		}
