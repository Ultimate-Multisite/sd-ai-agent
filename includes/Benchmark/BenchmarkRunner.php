...
class BenchmarkRunner {
	...
	public static function run_next_question( int $run_id ) {
		...
		if ( ! $run_config || ! is_array( $run_config ) ) {
			...
			return new \WP_Error(
				'benchmark_expired',
				__( 'Benchmark run configuration has expired. Please create a new run.', 'superdav-ai-agent' ),
				array( 'status' => 410 )
			);
		}
	...
	private static function call_wp_ai_client( string $provider_id, string $model_id, string $prompt ): array|\WP_Error {
		if ( ! function_exists( 'wp_ai_client_prompt' ) ) {
			return new \WP_Error(
				...
				__( 'WordPress AI Client not available. WordPress 7.0+ is required.', 'superdav-ai-agent' )
			);
		}
				...
				if ( ! $registry->hasProvider( $provider_id ) ) {
					return new \WP_Error(
						...
						sprintf(
							...
							__( 'Provider "%s" not found.', 'superdav-ai-agent' ),
							$provider_id
						)
					);
				}
					...
					if ( null === $model ) {
						return new \WP_Error(
							...
							sprintf(
								...
								__( 'Model "%1$s" not found for provider "%2$s".', 'superdav-ai-agent' ),
								$model_id,
								$provider_id
							)
						);
