	...
	public static function create_skill( string $slug, string $name, string $description, string $content ): int|WP_Error {
		...
		if ( $existing ) {
			return new WP_Error(
				...
				__( 'A skill with this slug already exists.', 'superdav-ai-agent' ),
				array( 'status' => 409 )
			);
		}
		...
		if ( false === $id ) {
			return new WP_Error(
				...
				__( 'Failed to create skill.', 'superdav-ai-agent' ),
				array( 'status' => 500 )
			);
		}
	...
	public static function delete_skill( int $id ): true|WP_Error {
		...
		if ( $result === 'builtin' ) {
			return new WP_Error(
				...
				__( 'Built-in skills cannot be deleted. You can disable them instead.', 'superdav-ai-agent' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( ! $result ) {
			return new WP_Error(
				...
				__( 'Failed to delete skill or skill not found.', 'superdav-ai-agent' ),
				array( 'status' => 500 )
			);
		}
