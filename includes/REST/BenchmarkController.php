...
final class BenchmarkController extends XWP_REST_Controller {
	...
	public function handle_get_suite( WP_REST_Request $request ) {
		...
		if ( ! $suite ) {
			return new WP_Error(
				...
				__( 'Benchmark suite not found.', 'superdav-ai-agent' ),
				array( 'status' => 404 )
			);
		}
	...
	public function handle_get_run( WP_REST_Request $request ) {
		...
		if ( ! $run ) {
			return new WP_Error(
				...
				__( 'Benchmark run not found.', 'superdav-ai-agent' ),
				array( 'status' => 404 )
			);
		}
	...
	public function handle_create_run( WP_REST_Request $request ) {
		...
		if ( empty( $models ) || ! is_array( $models ) ) {
			return new WP_Error(
				'benchmark_no_models',
				__( 'At least one model must be selected.', 'superdav-ai-agent' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $run_id ) {
			return new WP_Error(
				...
				__( 'Failed to create benchmark run.', 'superdav-ai-agent' ),
				array( 'status' => 500 )
			);
		}
	...
	public function handle_run_next( WP_REST_Request $request ) {
		...
		if ( ! $run ) {
			return new WP_Error(
				'benchmark_run_not_found',
				__( 'Benchmark run not found after execution.', 'superdav-ai-agent' ),
				array( 'status' => 404 )
			);
		}
	...
	public function handle_delete_run( WP_REST_Request $request ) {
		...
		if ( ! $deleted ) {
			return new WP_Error(
				...
				__( 'Failed to delete benchmark run.', 'superdav-ai-agent' ),
				array( 'status' => 500 )
			);
		}
	...
	public function handle_compare( WP_REST_Request $request ) {
		...
		if ( empty( $run_ids ) || ! is_array( $run_ids ) || count( $run_ids ) < 2 ) {
			return new WP_Error(
				...
				__( 'At least two benchmark runs must be selected for comparison.', 'superdav-ai-agent' ),
				array( 'status' => 400 )
			);
		}
