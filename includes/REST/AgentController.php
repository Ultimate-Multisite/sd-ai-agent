...
final class AgentController {
	...
	public function handle_create_agent( WP_REST_Request $request ) {
		...
		if ( ! $agent ) {
			return new WP_Error( 'sd_ai_agent_agent_not_found', __( 'Agent not found after creation.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_update_agent( WP_REST_Request $request ) {
		...
		if ( ! $agent ) {
			return new WP_Error( 'sd_ai_agent_agent_not_found', __( 'Agent not found after update.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
