...
final class SkillController extends XWP_REST_Controller {
	...
	public function handle_create_skill( WP_REST_Request $request ) {
		...
		if ( ! $skill ) {
			return new WP_Error( 'sd_ai_agent_skill_not_found', __( 'Skill not found after creation.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_update_skill( WP_REST_Request $request ) {
		...
		if ( ! $skill ) {
			return new WP_Error( 'sd_ai_agent_skill_not_found', __( 'Skill not found after update.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_reset_skill( WP_REST_Request $request ) {
		...
		if ( ! $skill ) {
			return new WP_Error( 'sd_ai_agent_skill_not_found', __( 'Skill not found after reset.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
