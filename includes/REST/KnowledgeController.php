...
final class KnowledgeController {
	...
	public function handle_create_collection( WP_REST_Request $request ) {
		...
		if ( ! $collection ) {
			return new WP_Error( 'sd_ai_agent_collection_not_found', __( 'Collection not found after creation.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_update_collection( WP_REST_Request $request ) {
		...
		if ( ! $collection ) {
			return new WP_Error( 'sd_ai_agent_collection_not_found', __( 'Collection not found after update.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_knowledge_upload( WP_REST_Request $request ) {
		...
		if ( empty( $files['file'] ) ) {
			return new WP_Error( 'sd_ai_agent_no_file', __( 'No file uploaded.', 'gratis-ai-agent' ), array( 'status' => 400 ) );
		}

		$collection_id = self::get_int_param( $request, 'collection_id' );

		if ( ! $collection_id ) {
			return new WP_Error( 'sd_ai_agent_no_collection', __( 'Collection ID is required.', 'gratis-ai-agent' ), array( 'status' => 400 ) );
		}

		$collection = KnowledgeDatabase::get_collection( $collection_id );
		if ( ! $collection ) {
			return new WP_Error( 'sd_ai_agent_collection_not_found', __( 'Collection not found.', 'gratis-ai-agent' ), array( 'status' => 404 ) );
		}
