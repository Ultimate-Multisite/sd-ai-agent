...
final class SessionController {
	...
	public function handle_bulk_sessions( WP_REST_Request $request ) {
		...
		switch ( $action ) {
			...
			case 'move':
				...
				break;
			default:
				return new WP_Error( 'sd_ai_agent_invalid_action', __( 'Invalid bulk action.', 'gratis-ai-agent' ), array( 'status' => 400 ) );
		}
	...
	public function handle_create_session( WP_REST_Request $request ) {
		...
		if ( ! $session ) {
			return new WP_Error( 'sd_ai_agent_session_not_found', __( 'Session not found after creation.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_update_session( WP_REST_Request $request ) {
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'sd_ai_agent_no_data', __( 'No fields to update.', 'gratis-ai-agent' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $session ) {
			return new WP_Error( 'sd_ai_agent_session_not_found', __( 'Session not found after update.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_export_session( WP_REST_Request $request ) {
		...
		if ( ! $session ) {
			return new WP_Error( 'sd_ai_agent_session_not_found', __( 'Session not found.', 'gratis-ai-agent' ), array( 'status' => 404 ) );
		}
	...
	public function handle_import_session( WP_REST_Request $request ) {
		...
		if ( empty( $data ) ) {
			return new WP_Error( 'sd_ai_agent_import_empty', __( 'No import data provided.', 'gratis-ai-agent' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $session ) {
			return new WP_Error( 'sd_ai_agent_session_not_found', __( 'Session not found after import.', 'gratis-ai-agent' ), array( 'status' => 500 ) );
		}
	...
	public function handle_confirm_tool( WP_REST_Request $request ) {
		...
		if ( ( $job['user_id'] ?? 0 ) !== get_current_user_id() ) {
			return new WP_Error( 'sd_ai_agent_forbidden', __( 'Not authorized.', 'gratis-ai-agent' ), array( 'status' => 403 ) );
		}
